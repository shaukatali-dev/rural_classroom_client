...
import { AreaChart, Area, ResponsiveContainer, Tooltip, Legend, CartesianGrid, XAxis, YAxis } from "recharts";
      ...
      <ResponsiveContainer>
        ...
        >
          ...
          {isVisible
            ? Object.keys(data[0])
                .filter((key) => key !== x)
                .map((y, i) => <Area key={i} type="monotone" name={y} dataKey={y} fill={colors[i]} stroke={colors[i]} strokeWidth={2} activeDot={{ r: 4 }} />)
            : null}
        </AreaChart>
      </ResponsiveContainer>
    </Box>
