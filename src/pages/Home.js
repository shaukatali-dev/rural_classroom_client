...
const Home = () => {
  ...
  const { user, token, courses, setCourses, lectures, tests, setTests } =
  ...
  const handleCourse = async (e) => {
    ...
    if (courseId) {
      ...
      if (coursePic) {
        setIsLoading(true);
        const url = await uploadPicture(coursePic, "course-pic");
        edits["coursePic"] = url;
      }
      try {
        setIsLoading(true);
        (courseId === "new"
          ? axios.post(COURSE_NEW_ENDPOINT, edits, {
              headers: { Authorization: `Bearer ${token}` },
            })
          : axios.patch(
              COURSE_EDIT_ENDPOINT,
              { query: { _id: courseId }, edits },
              { headers: { Authorization: `Bearer ${token}` } }
            )
        )
          .then((res) => {
  ...
  const handleTest = async (e) => {
    ...
    if (testId) {
      ...
      if (testPic) {
        setIsLoading(true);
        const url = await uploadPicture(testPic, "test-pic");
        edits["testPic"] = url;
      }
      try {
        setIsLoading(true);
        (testId === "new"
          ? axios.post(TEST_NEW_ENDPOINT, edits, {
              headers: { Authorization: `Bearer ${token}` },
            })
          : axios.patch(
              TEST_EDIT_ENDPOINT,
              { query: { _id: testId }, edits },
              { headers: { Authorization: `Bearer ${token}` } }
            )
        )
          .then((res) => {
  ...
  const handleCoursePic = (e) => {
    ...
    if (image.type.startsWith("image/")) {
      ...
      setCoursePic(null);
      alert(
        "Invalid file type! Upload an 'image' file as your course picture."
      );
    }
  };
            ...
            <Grid item xs={12}>
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} sm={4}>
                  <Autocomplete
                    name="lecture"
                    options={lectures.filter(
                      (lecture) => lecture.course === course?._id
                    )}
                    getOptionLabel={(option) => option.name}
                    renderInput={(params) => (
                      <TextField
